...
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.gold_dataset.provider_charge_summary` (
...
# truncate table
TRUNCATE TABLE `top-gantry-489113-j4.gold_dataset.provider_charge_summary`;

# insert data
INSERT INTO `top-gantry-489113-j4.gold_dataset.provider_charge_summary`
SELECT 
    CONCAT(p.firstname, ' ', p.LastName) AS Provider_Name,
    d.Name AS Dept_Name,
    SUM(t.Amount) AS Amount
FROM `top-gantry-489113-j4.silver_dataset.transactions` t
LEFT JOIN `top-gantry-489113-j4.silver_dataset.providers` p 
    ON SPLIT(p.ProviderID, "-")[SAFE_OFFSET(1)] = t.ProviderID
LEFT JOIN `top-gantry-489113-j4.silver_dataset.departments` d 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.gold_dataset.patient_history` (
    Patient_Key STRING,
    FirstName STRING,
    LastName STRING,
    Gender STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `top-gantry-489113-j4.gold_dataset.patient_history`;

# INSERT DATA
INSERT INTO `top-gantry-489113-j4.gold_dataset.patient_history`
SELECT 
    p.Patient_Key,
    p.FirstName,
    p.LastName,
    ...
    c.PayorType
FROM `top-gantry-489113-j4.silver_dataset.patients` p
LEFT JOIN `top-gantry-489113-j4.silver_dataset.encounters` e 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = e.PatientID
LEFT JOIN `top-gantry-489113-j4.silver_dataset.transactions` t 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = t.PatientID
LEFT JOIN `top-gantry-489113-j4.silver_dataset.claims` c 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.gold_dataset.provider_performance` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `top-gantry-489113-j4.gold_dataset.provider_performance`;

# INSERT DATA
INSERT INTO `top-gantry-489113-j4.gold_dataset.provider_performance`
SELECT 
    pr.ProviderID,
    pr.FirstName,
    pr.LastName,
...
FROM `top-gantry-489113-j4.silver_dataset.providers` pr
LEFT JOIN `top-gantry-489113-j4.silver_dataset.encounters` e 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.gold_dataset.department_performance` (
    Dept_Id STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `top-gantry-489113-j4.gold_dataset.department_performance`;

# INSERT DATA
INSERT INTO `top-gantry-489113-j4.gold_dataset.department_performance`
SELECT 
    d.Dept_Id,
...
FROM `top-gantry-489113-j4.silver_dataset.departments` d
LEFT JOIN `top-gantry-489113-j4.silver_dataset.encounters` e 
