...
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.silver_dataset.departments` (
    Dept_Id STRING,
    SRC_Dept_Id STRING,
    Name STRING,
    datasource STRING,
...
TRUNCATE TABLE `top-gantry-489113-j4.silver_dataset.departments`;

-- 3. full load by Inserting merged Data 
INSERT INTO `top-gantry-489113-j4.silver_dataset.departments`
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `top-gantry-489113-j4.bronze_dataset.departments_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `top-gantry-489113-j4.bronze_dataset.departments_hb`
);

-------------------------------------------------------------------------------------------------------

-- 1. Create table providers by Merge Data from Hospital A & B  
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.silver_dataset.providers` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
-- 2. Truncate Silver Table Before Inserting 
TRUNCATE TABLE `top-gantry-489113-j4.silver_dataset.providers`;

-- 3. full load by Inserting merged Data 
INSERT INTO `top-gantry-489113-j4.silver_dataset.providers`
SELECT DISTINCT 
    ProviderID,
    FirstName,
    LastName,
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `top-gantry-489113-j4.bronze_dataset.providers_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `top-gantry-489113-j4.bronze_dataset.providers_hb`
);

-------------------------------------------------------------------------------------------------------

-- 1. Create patients Table in BigQuery
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.silver_dataset.patients` (
    Patient_Key STRING,
...
CREATE OR REPLACE TABLE `top-gantry-489113-j4.silver_dataset.quality_checks` AS
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `top-gantry-489113-j4.bronze_dataset.patients_ha`
    ...
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosb' AS datasource
    FROM `top-gantry-489113-j4.bronze_dataset.patients_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `top-gantry-489113-j4.silver_dataset.patients` AS target
USING `top-gantry-489113-j4.silver_dataset.quality_checks` AS source
...
DROP TABLE IF EXISTS `top-gantry-489113-j4.silver_dataset.quality_checks`;

-------------------------------------------------------------------------------------------------------

-- 1. Create transactions Table in BigQuery
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.silver_dataset.transactions` (
...
SELECT DISTINCT 
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `top-gantry-489113-j4.bronze_dataset.transactions_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `top-gantry-489113-j4.bronze_dataset.transactions_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `top-gantry-489113-j4.silver_dataset.transactions` AS target
USING `top-gantry-489113-j4.silver_dataset.quality_checks` AS source
...
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.silver_dataset.encounters` (
...
CREATE OR REPLACE TABLE `top-gantry-489113-j4.silver_dataset.quality_checks_encounters` AS
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `top-gantry-489113-j4.bronze_dataset.encounters_ha`
    ...
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosb' AS datasource
    FROM `top-gantry-489113-j4.bronze_dataset.encounters_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `top-gantry-489113-j4.silver_dataset.encounters` AS target
USING `top-gantry-489113-j4.silver_dataset.quality_checks_encounters` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `top-gantry-489113-j4.silver_dataset.quality_checks_encounters`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the Claims Table in BigQuery
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.silver_dataset.claims` (
    Claim_Key STRING,
    SRC_ClaimID STRING,
...
CREATE OR REPLACE TABLE `top-gantry-489113-j4.silver_dataset.quality_checks_claims` AS
SELECT 
...
FROM (
    SELECT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `top-gantry-489113-j4.bronze_dataset.claims`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `top-gantry-489113-j4.silver_dataset.claims` AS target
USING `top-gantry-489113-j4.silver_dataset.quality_checks_claims` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `top-gantry-489113-j4.silver_dataset.quality_checks_claims`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the CP Codes Silver Table in BigQuery
CREATE TABLE IF NOT EXISTS `top-gantry-489113-j4.silver_dataset.cpt_codes` (
    CP_Code_Key STRING,
...
CREATE OR REPLACE TABLE `top-gantry-489113-j4.silver_dataset.quality_checks_cpt_codes` AS
SELECT 
...
FROM (
    SELECT 
        ...
        code_status,
        'hosa' AS datasource
    FROM `top-gantry-489113-j4.bronze_dataset.cpt_codes`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `top-gantry-489113-j4.silver_dataset.cpt_codes` AS target
USING `top-gantry-489113-j4.silver_dataset.quality_checks_cpt_codes` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `top-gantry-489113-j4.silver_dataset.quality_checks_cpt_codes`;
